...
| B05-S06 | [#49](https://github.com/AlexandreZanata/brain-root-idea/issues/49) | Version `0.0.1` synchronized across `VERSION`, `Cargo.toml`, `package.json`, and the lockfile; changelog `0.0.1` section; release artifact rebuilt (`BrainRoot_0.0.1_amd64.deb`, 3,383,162 bytes, sha256 `d03154c9…`); full CI on the frozen head; quality gates verified | pending (B05-S06 commit) | [evidence](https://github.com/AlexandreZanata/brain-root-idea/issues/49) | The first packaging run exposed a stale-artifact selection in `scripts/package-linux.sh`; fixed in this issue before the recorded identity | Closed |
...
- Artifact/checksum: `BrainRoot_0.0.1_amd64.deb`, 3,383,162 bytes, sha256 `d03154c9…` (per-build; `docs/specs/b05-linux-artifact.md`); attached to the pre-release at B05-S07
