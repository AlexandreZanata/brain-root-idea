...
- Status: **Shipped** — merged as [`9e2b51c`](https://github.com/AlexandreZanata/brain-root-idea/commit/9e2b51cc84687c6530ce4f7eb4993f40e07f0739) (2026-09-25T15:12:05Z) and released as annotated [`v0.0.15`](https://github.com/AlexandreZanata/brain-root-idea/releases/tag/v0.0.15); this record's remaining follow-ups are listed under Result and next batch
- Merge mechanics (read live, not assumed): `main` protection requires `check-full-linux` with `strict: true`, one approving review, and conversation resolution. `strict` is why the branch merges `main`; the review requirement cannot be met by a single maintainer identity, so — exactly as B17 ([#99](https://github.com/AlexandreZanata/brain-root-idea/pull/99)) and B18 ([#108](https://github.com/AlexandreZanata/brain-root-idea/pull/108)) did with zero recorded reviews — the merge goes through the documented administrator bypass. Auto-merge was enabled repository-wide and armed as `MERGE`, and it **did not fire**: with the required check green the pull request still reported `mergeable: MERGEABLE` with `mergeStateStatus: BLOCKED` and `reviewDecision: REVIEW_REQUIRED`, so it covers the status-check wait only. Recorded in `docs/specs/repository-rules-runbook.md`, not as a working approval process.
...
- Branch: `batch/b19-foundation`, pushed to `origin`, based on B18 tip `f4919f4`. `main` advanced to `1dbf7de`, and `git diff f4919f4 1dbf7de` is MEASURED empty, so the non-destructive merge from `main` required by `strict: true` was purely topological and changed no content — which is why it could be taken at the gate without re-litigating the gate evidence.
...
- Artifact: `BrainRoot_0.0.15_amd64.deb`, bytes 3,537,182, sha256 `ec15ec7cb2cfc2dcc782055a4e17a73629753a9f4158fce9984b3929152a4a01`. Checksum recorded; the artifact is **not** attached to the release page.
- Merge, tag, and release: merge commit `9e2b51c`, annotated tag `v0.0.15` on that commit, [release page](https://github.com/AlexandreZanata/brain-root-idea/releases/tag/v0.0.15) published as a pre-release, `VERSION` reads `0.0.15` on `main`
...
- Decision (maintainer, 2026-09-25): merge the batch and tag `0.0.15` **with the blocker open and declared**, on the condition that every surface calls the agent experimental and uncontained and nothing claims a boundary for it. Recorded as [ADR 0016](../../adr/0016-uncontained-agent-experiment.md). Shipping is not a resolution: the open question keeps its owner and the entry was not downgraded.
- Decision (maintainer, 2026-09-25): [ADR 0015](../../adr/draft-0015-opencode-serve-default.md) stays **Proposed**. ADR 0016 explicitly declines to accept the sidecar as the default architecture while shipping its implementation, so the draft is neither promoted nor silently treated as accepted.
- Decision (maintainer, 2026-09-25): `v0.0.15` is an annotated tag on the batch merge commit on `main`, created only after green latest-head CI, per `docs/19-release-and-versioning.md` — never on a branch head. This continues the B18 pattern of a merged-but-untagged version only until the tag exists.
- Decision (gate mechanics, 2026-09-25): repository auto-merge was enabled and armed so the merge does not require a waiting/polling task, with the caveat above about the review requirement. A repository setting changed outside the tree, so it is recorded in the runbook.
...
- CI: [`check-full-linux` **pass** on the gate head `2a07b9a`](https://github.com/AlexandreZanata/brain-root-idea/actions/runs/36151129872) (6m38s). The tested head is an ancestor of the merge commit and `git diff 2a07b9a 9e2b51c` is **empty**, so the required check covers exactly the content merged. Earlier snapshots, one per task and never polled: `7feda2c` → run 36149987256 cancelled by the reconciliation push (no verdict); `b4a5964` → run 36150364413 superseded by the `main` merge.
...
Batch B19 with the Lean YAGNI pivot **shipped as `0.0.15`**. The required check passed on the gate head `2a07b9a`, the merge commit `9e2b51c` carries that content unchanged, the annotated tag `v0.0.15` sits on the merge commit, and the release page is published. The merge used the documented administrator bypass because the required approving review cannot exist with a single maintainer identity; that is recorded as a gap, not as a working approval process.

Two things this batch did **not** resolve, and both are stated rather than implied. First, the agent still has **no workspace or permission boundary** and runs with the user's own authority — the blocker stays open in `docs/17-open-questions.md` with an owner, accepted for this release by [ADR 0016](../../adr/0016-uncontained-agent-experiment.md), and the first batch that touches the agent path should treat closing it as an acceptance criterion rather than a follow-up. Second, two `docs/19` pre-tag items are knowingly unmet and disclosed on the release page: the required approving review (bypassed) and the clean-room `.deb` install/remove probe, which requires root. The artifact's checksum is recorded but the artifact itself was not attached to the release.

Open follow-ups, in order: (1) close the agent workspace/permission boundary, or scope the agent out of any advertised capability until it closes; (2) decide whether the batch branch `batch/b19-foundation` should be deleted now that the merge is public; (3) if the `.deb` is to be published, run the clean-room install/run/close/remove probe first, since a release artifact is immutable once uploaded. Next batch: **B20 OpenCode visual parity** (issues [#130](https://github.com/AlexandreZanata/brain-root-idea/issues/130)–[#136](https://github.com/AlexandreZanata/brain-root-idea/issues/136) filed, none started), which supersedes the earlier B20 conversation/Canvas polish plan.
