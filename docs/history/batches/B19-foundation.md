...
The branch is versioned at `0.0.15`, pushed, up to date with `main`, and green on the local head. It is **mergeable only through the documented administrator bypass**: with a single maintainer identity the required approving review cannot exist, and auto-merge does not bypass reviews, so a green check alone will not complete the merge. Immediate next steps, in order: (1) let the required check finish on the up-to-date head — pending means report and stop, failed means a tracked remediation issue; (2) merge with a merge commit through the administrator bypass, exactly as B17 ([#99](https://github.com/AlexandreZanata/brain-root-idea/pull/99)) and B18 ([#108](https://github.com/AlexandreZanata/brain-root-idea/pull/108)) did, the "up to date" rule already being satisfied by the `main` merge; (3) place the annotated `v0.0.15` tag on that merge commit and publish the release page restating the uncontained-agent limitation; (4) record the merge SHA, the tag, and the final CI result in this record and the Wiki — nothing is pre-filled as passing. The agent permission-boundary blocker stays **open** with its owner under [ADR 0016](../../adr/0016-uncontained-agent-experiment.md); the first batch that touches the agent path should treat closing it as an acceptance criterion rather than a follow-up. Next product batch: B20 conversation/Canvas polish, only after this merges.
