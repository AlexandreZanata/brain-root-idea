...
- **Human browser policy:** the role, navigation, profile ownership/isolation, denied permissions, lifecycle, and retention boundaries are decided in [ADR 0013](adr/0013-human-browser-policy.md) (arbitrary `http`/`https`, no downloads/popups/permission prompts in the first slice). Measured 2026-09-23 (B13-S02): two role profiles with separate `WebContext` data directories are isolated in both directions, persist across destroy/recreate, live inside the application data directory, and clean up ([isolation probe](specs/human-profile-isolation-probe.md)). Still open: whether a curated-destination mode is ever worth adding, the exact future user-mediated share flow and its threat model, the concrete retention/delete UX, and cookie/cache/deletion/crash behavior for the implementation batch; the implementation is post-MVP (Phase 8) and now gated only on that batch.
