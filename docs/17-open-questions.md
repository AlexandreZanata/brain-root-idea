...
- **Human browser policy:** the role, navigation, profile ownership/isolation, denied permissions, lifecycle, and retention boundaries are decided in [ADR 0013](adr/0013-human-browser-policy.md) (arbitrary `http`/`https`, no downloads/popups/permission prompts in the first slice). Still open: whether a curated-destination mode is ever worth adding, the exact future user-mediated share flow and its threat model, and the concrete retention/delete UX; the implementation is post-MVP (Phase 8) and gated on the B13-S02 profile isolation probe.
