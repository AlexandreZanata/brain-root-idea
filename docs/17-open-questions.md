...
- **OpenCode Go credential UX:** Linux Secret Service availability, recovery when unavailable, subscription onboarding, and whether BrainRoot may delegate authentication to an installed OpenCode client without reading its plaintext auth file. Owner: MVP-0 security spike.
...
- **Async/runtime choice:** Smallest Rust concurrency stack consistent with Tauri, cancellation, and process I/O.
