...
6. Mark the draft PR Ready for review. This triggers the full required Linux CI.
7. Do not merge until CI passes on the latest head, required review approves, conversations are resolved, and Wiki/history links are present.
8. If CI fails, open a remediation issue in the same batch. Fix it as a normal microstep and rerun the full gate.
9. Merge with a merge commit so the one-commit-per-issue history remains visible. Delete the batch branch after the merge.
10. Create the annotated tag/release only after the merge commit exists on `main`.
