...
The Human Browser has its own approved policy in [ADR 0013](adr/0013-human-browser-policy.md): a separate persistent BrainRoot-owned profile that agents cannot read, user-directed `http`/`https`, denied schemes, popups, downloads, and permission prompts in the first slice, and no ambient agent access. Its implementation is post-MVP and gated on the profile isolation probe (B13-S02).
...
**Maintainer decision (2026-09-25, [ADR 0016](adr/0016-uncontained-agent-experiment.md)):** `0.0.15` ships and is tagged with this gap **open and declared**, on the grounds that the sidecar starts only on an explicit user action. The decision does not narrow the gap. It binds every surface that describes the agent — README, changelog, this file, the Wiki batch page, and the release page — to call it experimental and uncontained, forbids any Safe Mode, sandbox, or permission-boundary claim for agent actions, keeps the sidecar's bounded lifetime and explicit start, and leaves the question open with an owner until a canonicalized approved root, a permission moment, denial tests, and either an OS/container boundary or a truthful enforcement-floor statement exist. The residual risk is the user's own authority: a malicious repository, a prompt injection in fetched content, or an agent mistake can read or modify anything the account can. Explicit start is a speed bump, not a boundary, and this disclosure is not a mitigation.
